use log::*;
use screeps::{find, game, Creep, Deposit, HasId, HasPosition, ObjectId, Part, Position, ResourceType,
    SharedCreepProperties, Tombstone
};
use crate::{
    units::{Task, TaskResult, roles::Role, has_part, tasks::FLEE_RANGE},
    movement::walker::Walker,
    utils::{commons::{closest_attacker, get_place_to_store}, constants::CLOSE_RANGE_ACTION}
};

pub fn deposit_mine(pos: Position, id: ObjectId<Deposit>, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
        ...
        .filter(|enemy| enemy.pos().get_range_to(creep.pos()) <= 4)
    {
        ...
        if let Some(deposit) = id.resolve() {
            if deposit.cooldown() == 0 && creep.store().get_free_capacity(Some(deposit.deposit_type())) > 0 {
...
pub fn deposit_carry(pos: Position, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
    ...
    if is_full(creep) || (creep.ticks_to_live().is_some_and(|ticks| ticks < 200) && creep.store().get_used_capacity(None) > 0) {
        if let Some(container) = get_place_to_store(&home_room) {
            ...
            let resource_type = resources.first()
            ...
            TaskResult::StillWorking(
                Task::DeliverToStructure(container.pos(), container.as_structure().raw_id(), *resource_type, None),
                Some(goal))
        } else {
        ...
        let goal = Walker::Exploring(false).walk(tomb.pos(), CLOSE_RANGE_ACTION, creep, role, enemies);
        TaskResult::StillWorking(Task::TakeFromStructure(tomb.pos(), tomb.raw_id(), resource, None), Some(goal))
        ...
        let goal = Walker::Exploring(true).walk(miner.pos(), CLOSE_RANGE_ACTION, creep, role, enemies);
        TaskResult::StillWorking(Task::DepositCarry(pos), Some(goal))
    } else if let Some(in_range_attacker) = closest_attacker(creep, enemies.iter())
        .filter(|enemy| enemy.pos().get_range_to(creep.pos()) <= 4)
    {
...
fn loaded_miner(creep: &Creep) -> Option<Creep> {
    ...
    room.find(find::MY_CREEPS, None)
        .into_iter()
        .find(|miner| has_part(&[Part::Work], miner, false)
            && miner.store().get_used_capacity(None) > 0)
}

fn loaded_tomb(creep: &Creep) -> Option<(Tombstone, ResourceType)> {
    let room = creep.room().expect("expect creep is in a room!");
    room.find(find::TOMBSTONES, None)
        .into_iter()
        .find_map(|tomb| {
            tomb.store().store_types().iter()
...
fn get_near_by_hauler(to: &Creep) -> Option<Creep> {
    ...
    room.find(find::MY_CREEPS, None)
        .into_iter()
        .find(|creep| creep.pos().is_near_to(to.pos()) &&
