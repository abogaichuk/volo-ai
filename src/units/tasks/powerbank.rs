...
use log::*;
use screeps::{find, game, Creep, Ruin, HasId, HasPosition, ObjectId,
    Position, ResourceType, Room, SharedCreepProperties, StructurePowerBank
};
use crate::{
    units::{
        Task, TaskResult, roles::Role,
        tasks::{logistics::{take_from_structure, deliver_to_structure, take_resource}, FLEE_RANGE}
    },
    movement::walker::Walker,
    utils::{
        commons::{closest_attacker, find_dropped, get_place_to_store},
        constants::{CLOSE_RANGE_ACTION, LONG_RANGE_ACTION}
    }
};

pub fn pb_attack(pos: Position, id: ObjectId<StructurePowerBank>, members: HashSet<String>, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
    if let Some(healer) = members.iter()
        ...
        (creep.pos().is_near_to(healer.pos()) || creep.pos().is_room_edge())
    {
            ...
            .filter(|enemy| enemy.pos().get_range_to(creep.pos()) <= 4)
        {
            ...
            .filter(|enemy| enemy.pos().get_range_to(creep.pos()) <= 7).is_some()
        {
...
pub fn pb_heal(pos: Position, id: ObjectId<StructurePowerBank>, members: HashSet<String>, creep: &Creep, role: &Role, enemies: Vec<Creep>) -> TaskResult {
    if let Some(pb_attacker) = members.iter()
        .find(|name| **name != creep.name())
        .and_then(|member| game::creeps().get(member.clone()))
    {
        //if member found
...
pub fn pb_carry(pos: Position, id: ObjectId<StructurePowerBank>, creep: &Creep, role: &Role, enemies: Vec<Creep>) -> TaskResult {
    ...
    if creep.store().get_used_capacity(Some(ResourceType::Power)) > 0 {
        if let Some(storage) = get_place_to_store(&home_room) {
            match deliver_to_structure(storage.pos(), storage.as_structure().raw_id(), ResourceType::Power, None, creep, role, enemies) {
        ...
        .is_some()
    {
        ...
        if id.resolve().is_none() {
            ...
            if let Some(ruin) = find_ruin(&room) {
                take_from_structure(ruin.pos(), ruin.raw_id(), ResourceType::Power, None, creep, role, enemies)
