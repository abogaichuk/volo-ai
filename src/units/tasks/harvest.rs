use log::*;
use screeps::{Creep, HasHits, HasId, HasPosition, Mineral, ObjectId,
    Position, ResourceType, SharedCreepProperties, Source,
    StructureController, StructureObject, StructureType,
    action_error_codes::HarvestErrorCode, find, look
};
use crate::{
    units::{Task, TaskResult, roles::Role, need_escape, tasks::FLEE_RANGE},
    movement::walker::Walker,
    utils::commons::{closest_attacker, find_closest_exit, find_container_near_by, find_cs_near_by, has_enough_space}
};
...
pub fn harvest_energy_forever(workplace: Position, id: ObjectId<Source>, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
        ...
        .filter(|enemy| enemy.pos().get_range_to(creep.pos()) <= 4)
    {
        ...
        if creep.store().get_used_capacity(Some(ResourceType::Energy)) > half_capacity(creep) {
            if let Some(link) = workplace.find_in_range(find::MY_STRUCTURES, 1).into_iter()
                .find_map(|structure| match structure {
                    StructureObject::StructureLink(l) if l.store().get_free_capacity(Some(ResourceType::Energy)) > 0 => Some(l),
                    _ => None
                })
            {
            ...
            } else if let Some(container) = workplace.look_for(look::STRUCTURES).ok()
                .and_then(|structures| structures.into_iter()
                    .find_map(|structure| match structure {
...
pub fn harvest_minerals(workplace: Position, id: ObjectId<Mineral>, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
...
pub fn harvest_until_full(workplace: Position, id: ObjectId<Source>, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
    if !creep.pos().is_equal_to(workplace) {
        ...
        if let Some(source) = id.resolve() {
            ...
            match creep.harvest(&source) {
                ...
                Err(err) => {
                    match err {
                        HarvestErrorCode::NotEnoughResources if better_work(&source, creep) => TaskResult::Abort,
            ...
            warn!("{} source {} near: {} can't be resolved!", creep.name(), id.to_string(), workplace);
            TaskResult::Abort
        }
    } else {
...
fn better_work(source: &Source, creep: &Creep) -> bool {
    if let Some(ticks) = source.ticks_to_regeneration() &&
        (ticks > 100 ||
        ticks > 30 && creep.store().get_used_capacity(Some(ResourceType::Energy)) > creep.store().get_capacity(None) / 2)
    {
        true
    } else {
        false
...
pub fn harvest_and_upgrade(workplace: Position, id: ObjectId<Source>, ctrl_id: ObjectId<StructureController>, creep: &Creep, role: &Role, enemies:Vec<Creep>) -> TaskResult {
    if !creep.pos().is_equal_to(workplace) {
        let goal = Walker::Exploring(false).walk(workplace, 0, creep, role, enemies);
...
fn is_full(creep: &Creep) -> bool {
    creep.store().get_used_capacity(None) >= creep.store().get_capacity(None) - creep.store().get_capacity(None) / 25 // 4%
}

fn transfer_or_drop(workplace: Position, creep: &Creep) {
    if let Some(resource) = creep.store().store_types().first() {
        match find_container_near_by(&workplace, 1, &[StructureType::Link, StructureType::Container]) {
            ...
            Some(StructureObject::StructureLink(l))
                ...
                has_enough_space(&l, creep.store().get_used_capacity(Some(ResourceType::Energy))) =>
            {
                let _ = creep.transfer(&l, ResourceType::Energy, None);
            }
            _ => { let _ = creep.drop(*resource, None); }
        };
    }
}