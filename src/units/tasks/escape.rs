...
use crate::{
    ...
    utils::{
        commons::is_walkable, constants::{ESCAPE_FROM_EDGE_RANGE, HIDE_TIMEOUT}
    }
};


pub fn hide(position: Position, timeout: u32, creep: &Creep, role: &Role, enemies: Vec<Creep>) -> TaskResult {
...
pub fn escape(position: Position, creep: &Creep, role: &Role, enemies: Vec<Creep>) -> TaskResult {
    ...
    if creep.pos().is_room_edge() && position.is_room_edge() && creep.pos().room_name() != position.room_name() {
...
fn find_walkable_positions_remoted_from(position: Position, range: u32) -> Option<Position> {
    get_positions_ranged_from(position, range)
        .find(|pos| !pos.is_room_edge() && is_walkable(pos))
}

fn all_positions(room_name: RoomName) -> impl Iterator<Item = Position> {
    (1..49).flat_map(move |x| (1..49).clone()
        .map(move |y| unsafe {
            Position::new(RoomCoordinate::unchecked_new(x), RoomCoordinate::unchecked_new(y), room_name)
        }))
}

fn get_positions_ranged_from(position: Position, range: u32) -> impl Iterator<Item = Position> {
    all_positions(position.room_name())
        .filter(move |pos| pos.get_range_to(position) == range)
}