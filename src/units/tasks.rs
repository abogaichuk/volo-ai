...
use screeps::{
    find, ConstructionSite, Creep, Deposit, HasPosition, Mineral, ObjectId,
    Part, Position, RawObjectId, Resource, ResourceType, RoomCoordinate, RoomName,
    Source, Structure, StructureController, StructureInvaderCore, StructureLab,
    StructurePowerBank, SOURCE_KEEPER_USERNAME
};
        ...
        match self {
            ...
            Task::MoveMe(room_name, walker) => {
                if creep.pos().room_name() == room_name {
                    ...
                    let _ = creep.say("🚶🏻", true);
                    let position = unsafe {
                        Position::new(RoomCoordinate::unchecked_new(25), RoomCoordinate::unchecked_new(25), room_name)
                    };
            ...
            Task::Provoke(ticks, range) => {
                let attackers: Vec<Creep> = hostiles.into_iter()
                ...
                if let Some(closest_hostile) = attackers.first() {
                    ...
                    match actual_range.cmp(&range) {
                        Ordering::Less => {
                            let goal = Walker::Flee.walk(closest_hostile.pos(), 10, creep, role, attackers);
                            TaskResult::StillWorking(Task::Provoke(5, 10), Some(goal))
                        }
                        Ordering::Greater => {
                            let goal = Walker::Exploring(false).walk(closest_hostile.pos(), range, creep, role, attackers);
                ...
                if let Some(closest_hostile) = closest_attacker(creep, attackers.iter()) {
                    if creep.pos().get_range_to(closest_hostile.pos()) < 6 {
                        ...
                        if closest_hostile.owner().username() != SOURCE_KEEPER_USERNAME &&
                            ...
                            creep.pos().get_range_to(exit) < creep.pos().get_range_to(closest_hostile.pos())
                        {
                            //todo stuck when all cells danger, then vec::new to avoid setting danger cells
                            let goal = Walker::Exploring(false).walk(exit, 0, creep, role, Vec::new());
                            TaskResult::StillWorking(Task::Escape(exit), Some(goal))
                        } else {
                            //todo stuck when all cells danger, then vec::new to avoid setting danger cells
                            let goal = Walker::Flee.walk(closest_hostile.pos(), range, creep, role, Vec::new());
            ...
            Task::Escape(position) => escape::escape(position, creep, role, with_parts(hostiles, vec![Part::RangedAttack, Part::Attack])),
            Task::Hide(position, timeout) => escape::hide(position, timeout, creep, role, with_parts(hostiles, vec![Part::RangedAttack, Part::Attack])),
            Task::Boost(id, parts_number) => boost::boost(id, parts_number, creep, role),
            Task::Harvest(workplace, id) => {
                harvest::harvest_until_full(workplace, id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::HarvestAndUpgrade(workplace, id, ctrl_id) => {
                harvest::harvest_and_upgrade(workplace, id, ctrl_id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::HarvestEnergyForever(workplace, id) => {
                harvest::harvest_energy_forever(workplace, id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::HarvestMineral(workplace, id) => {
                harvest::harvest_minerals(workplace, id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::PowerbankHeal(pos, id, members) => {
                powerbank::pb_heal(pos, id, members, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack]))
            }
            Task::PowerbankAttack(pos, id, members) => {
                powerbank::pb_attack(pos, id, members, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack]))
                    // with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::PowerbankCarry(pos, id) => {
                powerbank::pb_carry(pos, id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack]))
            }
            Task::DepositHarvest(pos, id) => {
                deposit::deposit_mine(pos, id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::DepositCarry(pos) => {
                deposit::deposit_carry(pos, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::Build(id, pos) => {
                build::build(id, pos, creep, role, hostiles)
            }
            Task::Repair(id, pos, times) => {
                repair::repair(id, pos, times, creep, role, hostiles)
            }
            Task::Upgrade(id, container_id) => {
                upgrade::upgrade(id, container_id, creep, role, with_parts(hostiles, vec![Part::Attack, Part::RangedAttack]))
            },
            Task::Book(id, position) => {
                reservation::book(id, position, creep, role, with_parts(hostiles, vec![Part::Attack, Part::RangedAttack]))
            }
            Task::Claim(id, position) => {
                reservation::claim(id, position, creep, role, with_parts(hostiles, vec![Part::Attack, Part::RangedAttack]))
            }
            Task::PullTo(creep_name, destination) => {
                logistics::pull_to(creep_name, destination, creep, role, with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::Dismantle(id, workplace) => {
                dismantle::dismantle(id, workplace, creep, role)
            }
            Task::TakeResource(id) => {
                logistics::take_resource(id, creep, role, with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            },
            Task::TakeFromStructure(pos, id, resources, room_reqested) => {
                logistics::take_from_structure(pos, id, resources, room_reqested, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            ...
            Task::GenerateSafeMode(pos, id, storage_id) => {
                logistics::generate_safe_mode(pos, id, storage_id, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::LongRangeWithdraw(pos, id, resource, amount) => {
                logistics::long_range_withdraw(pos, id, resource, amount, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::Carry(from, to, resource, amount, _) => {
                logistics::carry(from, to, resource, amount, creep, role,
                    with_parts(hostiles, vec![Part::RangedAttack, Part::Attack]))
            }
            Task::DefendHome => combat::defend_home(creep, role, hostiles),
            Task::Crash(id, pos) => {
                combat::crash(id, pos, creep, role, hostiles)
            }
            Task::FindHeal(room_name) => {
                combat::find_heal(room_name, creep, role, hostiles)
            }
...
impl fmt::Debug for Task {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self {
            ...
            Task::MoveMe(room_name, walk_type) => write!(f, "Task::MoveMe[{}, {:?}]", room_name, walk_type),
            ...
            Task::Upgrade(id, container_id) => write!(f, "Task::Upgrade[{}, {:?}]", id, container_id),
            Task::TakeResource(id) => write!(f, "Task::TakeResource[{}]", id),
            Task::FindHeal(room_name) => write!(f, "Task::FindHeal[{}]", room_name),
            Task::Oversee(room_name, target) => write!(f, "Task::Oversee[{}, {:?}]", room_name, target),
            Task::Boost(id, parts) => write!(f, "Task::Boost[{}, {:?}]", id, parts),
            Task::Crash(id, pos) => write!(f, "Task::Crash[{}, {:?}]", id, pos),
            Task::Defend(room_name, room_requested) => write!(f, "Task::Defend[{}, {}]", room_name, room_requested),
            ...
            Task::HarvestEnergyForever(id, pos) => write!(f, "Task::HarvestEnergyForever[{}, {}]", id, pos),
            Task::HarvestMineral(id, pos) => write!(f, "Task::HarvestMineral[{}, {}]", id, pos),
            Task::Harvest(pos, id) => write!(f, "Task::Harvest[{}, {}]", id, pos),
            Task::HarvestAndUpgrade(pos, id, ctrl_id) => write!(f, "Task::HarvestAndUpgrade[{}, {}, {}]", pos, id, ctrl_id),
            Task::PowerbankAttack(pos, id, members) => write!(f, "Task::PowerbankAttack[{}, {}, {:?}]", id, pos, members),
            Task::PowerbankHeal(pos, id, members) => write!(f, "Task::PowerbankHeal[{}, {}, {:?}]", id, pos, members),
            ...
            Task::TakeFromStructure(pos, id, resource, amount) => write!(f, "Task::TakeFromStructure[{}, {}, {}, {:?}]", pos, id, resource, amount),
...
impl From<RequestKind> for Task {
    fn from(request_kind: RequestKind) -> Self {
        match request_kind {
            ...
            RequestKind::LongRangeWithdraw(r) => Task::LongRangeWithdraw(r.pos, r.id, r.resource, r.amount),
...
pub enum TaskResult {
    ...
    UpdateRequest(Task), //update room request (in the middle of doing something, partially carried resource or repair structure)
