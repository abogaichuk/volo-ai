...
impl Kind for Carrier {
    fn get_task(&self, creep: &Creep, home: &mut Shelter) -> Task {
        //todo bug here fix Long Range withdraw to carry back it to storage, long range carry!
        if creep.store().get_used_capacity(None) > 0 {
            ...
            let Some(mut request) = get_request(home, creep)
...
fn get_request(home: &Shelter, creep: &Creep) -> Option<Request> {
    home.requests()
        .find(|r| matches!(&r.kind, RequestKind::LongRangeWithdraw(_) if
