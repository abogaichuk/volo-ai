...
use screeps::{objects::Creep, HasId, Part, ResourceType, RoomName};
...
impl Kind for Fighter {
    ...
    fn body(&self, room_energy: u32) -> ArrayVec<[Part; 50]> {
        let (basic_parts, scale_parts) = if self.boost {
            (
                [Part::RangedAttack, Part::RangedAttack, Part::RangedAttack, Part::RangedAttack, Part::RangedAttack],
                [Part::Heal, Part::Heal, Part::Tough, Part::Tough, Part::Move, Part::Move, Part::RangedAttack, Part::RangedAttack, Part::RangedAttack]
            )
        } else {
            (
                [Part::Move, Part::Move, Part::Move, Part::Move, Part::Move],
                [Part::Tough, Part::RangedAttack, Part::RangedAttack, Part::Move, Part::Move, Part::Move, Part::Move, Part::Heal, Part::Heal]
            )
        };
    ...
    fn boosts(&self, creep: &Creep) -> HashMap<Part, [ResourceType; 2]> {
        ...
        if self.boost && creep.ticks_to_live().is_some_and(|tick| tick > 1000) {
            [
                (Part::Move, [ResourceType::CatalyzedZynthiumAlkalide, ResourceType::ZynthiumAlkalide]),
                (Part::RangedAttack, [ResourceType::CatalyzedKeaniumAlkalide, ResourceType::KeaniumAlkalide]),
                (Part::Heal, [ResourceType::CatalyzedLemergiumAlkalide, ResourceType::LemergiumAlkalide]),
                (Part::Tough, [ResourceType::CatalyzedGhodiumAlkalide, ResourceType::GhodiumAlkalide])
            ].into()
        } else {
            HashMap::new()
        }
    ...
    fn get_task(&self, creep: &Creep, home: &mut Shelter) -> Task {
        home.get_available_boost(creep, self.boosts(creep))
            ...
            .or_else(|| self.target
                .map(|target| Task::Protect(target, None)))
            .unwrap_or_default()
    }
}