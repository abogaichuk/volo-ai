...
use screeps::{Creep, Part, HasId, HasPosition, ResourceType, RoomName, SharedCreepProperties};
use std::fmt;
use arrayvec::ArrayVec;
use crate::{
    movement::MovementProfile,
    rooms::{shelter::Shelter, state::requests::{Request, RequestKind, meta::Status}},
    utils::constants::MIN_ENERGY_CAPACITY
};
    ...
    fn get_task(&self, creep: &Creep, home: &mut Shelter) -> Task {
        get_active_job(home, creep)
            ...
            .or_else(|| (creep.store().get_used_capacity(None) > 0)
                ...
                .then(|| {
                    let resource = creep.store().store_types()
                        .into_iter().next().expect("expect something in the creep store");
                    if let Some(sender) = home.empty_sender() && resource == ResourceType::Energy {
                        //if resource energy and sender link is empty -> fill it
                        Task::DeliverToStructure(sender.pos(), sender.raw_id(), ResourceType::Energy, None)
                    } else if let Some(bank) = home.storage()
            ...
            .or_else(|| home.full_receiver()
                //empty creep here and receiver link is full -> take all energy from receiver
                .map(|link| Task::TakeFromStructure(link.pos(), link.raw_id(), ResourceType::Energy, None)))
            .or_else(|| home.empty_sender().is_some()
                .then(|| home.storage()
                    .filter(|s| s.store().get_used_capacity(Some(ResourceType::Energy)) > MIN_ENERGY_CAPACITY)
            ...
            .or_else(|| (creep.ticks_to_live().is_some_and(|ticks| ticks > 10))
                .then(|| get_new_job(home)
...
fn get_active_job(home: &Shelter, creep: &Creep) -> Option<Request> {
    home.requests()
        .find(|r| matches!(&r.kind, RequestKind::Carry(_) if
