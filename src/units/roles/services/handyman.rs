...
use screeps::{
    constants::look::STRUCTURES, find, game, objects::Creep, prelude::*, ConstructionSite,
    Part, Position, ResourceType, Room, RoomName, StructureObject, StructureRampart,
};
...
use crate::{
    ...
    commons::{find_hostiles, find_source_near, remoted_from_edge},
    utils::constants::{HANDYMAN_ENERGY_PICKUP_THRESHOLD, NO_TASK_IDLE_TICKS}
};
...
impl Kind for HandyMan {
    ...
    fn body(&self, room_energy: u32) -> ArrayVec<[Part; 50]> {
        let (basic_parts, scale_parts): (SmallVec<[Part; 3]>, SmallVec<[Part; 7]>) = if self.boost {
            (
                smallvec![Part::Carry],
                smallvec![Part::Work, Part::Move, Part::Work, Part::Move, Part::Work, Part::Move, Part::Carry],
            )
        } else {
            (
    ...
    fn get_task(&self, creep: &Creep, home: &mut Shelter) -> Task {
        home.get_available_boost(creep, self.boosts(creep))
            ...
            .or_else(|| self.workplace
                .map(|workplace| {
                    if creep.pos().room_name() == workplace.room_name() &&
                        let Some(room) = game::rooms().get(workplace.room_name())
                    {
                        let hostiles: Vec<Creep> = find_hostiles(&room, vec![Part::RangedAttack, Part::Attack]).collect();
                        ...
                        Task::MoveMe(workplace.room_name(), crate::movement::walker::Walker::Exploring(false))
                    }
                }))
            .unwrap_or_default()
    }
...
fn get_rampart(workplace: Position) -> Option<StructureRampart> {
    workplace.look_for(STRUCTURES).into_iter()
        .find_map(|structures| {
...
fn build_near(creep :&Creep, room: &Room) -> Option<Task> {
    room.find(find::MY_CONSTRUCTION_SITES, None).iter()
        .find_map(|c| {
...
fn repair_near(creep :&Creep, room: &Room) -> Option<Task> {
    room.find(find::STRUCTURES, None).iter()
        .filter_map(|s| {
            if s.pos().get_range_to(creep.pos()) < 4 {
                match s {
                    ...
                    StructureObject::StructureContainer(c) if c.hits() < (c.hits_max() as f32 * 0.75) as u32 => {
            ...
            match s {
                StructureObject::StructureTower(t)
                    if t.pos().is_near_to(creep.pos()) &&
                        t.store().get_free_capacity(Some(ResourceType::Energy)) > (t.store().get_capacity(Some(ResourceType::Energy)) / 2) as i32 =>
                            Some(Task::DeliverToStructure(s.pos(), s.as_structure().raw_id(), ResourceType::Energy, None)),
                _ => None
            }
        })
}
...
fn find_startup_task(creep: &Creep, room: &Room) -> Task {
    ...
    for structure in room.find(find::STRUCTURES, None) {
        ...
        if store.get_free_capacity(Some(ResourceType::Energy)) > (store.get_capacity(Some(ResourceType::Energy)) / 2) as i32 {
            return Task::DeliverToStructure(structure.pos(), structure.as_structure().raw_id(), ResourceType::Energy, None)
        }
    }
    ...
    for structure in room.find(find::STRUCTURES, None) {
        match structure {
            // StructureObject::StructureRampart(r) if r.my() && r.hits() < (r.hits_max() as f32 * 0.5) as u32 => {
            // StructureObject::StructureRampart(r) if r.my() && r.hits() < 10000000 => {
            ...
            // },
            StructureObject::StructureContainer(c) if c.hits() < ((c.hits_max() as f32 * 0.5) as u32) => {
    ...
    if let Some(construction_site) = room
        ...
        .into_iter()
        .fold(None, |acc: std::option::Option<ConstructionSite>, another| {
            if let Some(cs) = acc {
                match another.pos().get_range_to(creep.pos()).cmp(&cs.pos().get_range_to(creep.pos())) {
...
fn find_energy_near(creep :&Creep, _: StructureRampart, room: &Room, _: &[Creep]) -> Task {
    dropped_near(room, creep.pos())
        ...
        .or_else(|| find_source_near(creep.pos(), room).map(|source| Task::Harvest(creep.pos(), source.id())))
        .unwrap_or(Task::Idle(1))
}

fn dropped_near(room: &Room, workplace: Position) -> Option<Task> {
    room.find(find::DROPPED_RESOURCES, None).iter()
        .find(|resource| resource.pos().is_near_to(workplace) &&
...
fn full_container_near(workplace: Position, room: &Room) -> Option<Task> {
    room.find(find::STRUCTURES, None).iter()
        .find_map(|s| {
            match s {
                StructureObject::StructureContainer(c) if c.pos().is_equal_to(workplace) &&
...
fn find_energy_or_source(room: &Room, workplace: Position) -> Task {
    ...
    for structure in room.find(find::STRUCTURES, None) {
        ...
        {
            return Task::TakeFromStructure(structure.pos(), structure.as_structure().raw_id(), ResourceType::Energy, None)
        }
    }

    if let Some(source) = find_source_near(workplace, room) {
        if let Some(ctrl) = room.controller() && ctrl.pos().get_range_to(workplace) <= 3 {
