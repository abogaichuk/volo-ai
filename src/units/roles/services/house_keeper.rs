...
use screeps::{
    Creep, HasId, HasPosition, Part, ResourceType, RoomName, SharedCreepProperties, StructureObject, StructureType::{Extension, Factory, Lab, Link, Nuker, PowerSpawn, Spawn, Tower}, find
};
...
impl Kind for HouseKeeper {
    ...
    fn boosts(&self, creep: &Creep) -> HashMap<Part, [ResourceType; 2]> {
        if creep.ticks_to_live().is_some_and(|tick| tick > 800) {
            [(Part::Work, [ResourceType::CatalyzedLemergiumAcid, ResourceType::LemergiumAcid])].into()
        } else {
            HashMap::new()
        }
    }
    ...
    fn get_task(&self, creep: &Creep, home: &mut Shelter) -> Task {
        home.get_available_boost(creep, self.boosts(creep))
            ...
            .or_else(|| (creep.store().get_used_capacity(Some(ResourceType::Energy)) == 0)
                .then(|| find_energy(creep, home)))
            .or_else(|| (home.invasion())
                .then(|| home.lowest_perimetr_hits()
...
fn find_energy(creep: &Creep, home: &Shelter) -> Task {
    (creep.store().get_free_capacity(Some(ResourceType::Energy)) > 0)
        .then(|| get_closest_energy_container(creep)
            .map(|o| Task::TakeFromStructure(o.pos(), o.as_structure().raw_id(), ResourceType::Energy, None)))
            .flatten()
        .unwrap_or_else(|| home.storage()
            .filter(|storage| storage.store().get_used_capacity(Some(ResourceType::Energy)) > 10000)
...
fn get_closest_energy_container(creep: &Creep) -> Option<StructureObject> {
    ...
    let mut  energy_containers:Vec<StructureObject> = room.find(find::STRUCTURES, None)
        .into_iter()
        .filter(|s| {
            let s_type = s.as_structure().structure_type();
            s_type != Tower && s_type != Link && s_type != Extension && s_type != Spawn && s_type != Lab && s_type != PowerSpawn && s_type != Nuker && s_type != Factory && has_energy(s)
        })
        .collect();
...
fn get_active_job(home: &Shelter, creep: &Creep) -> Option<Request> {
    home.requests()
        .find(|r| {
            match &r.kind {
                RequestKind::Repair(_) if matches!(*r.status(), Status::InProgress) && r.assigned_to(&creep.name()) => true,
                RequestKind::Build(_) if matches!(*r.status(), Status::InProgress) && r.assigned_to(&creep.name()) => true,
                _ => false
            }
        })
        .cloned()
...
fn get_new_job(home: &Shelter) -> Option<Request> {
    get_earlier_build(home)
        .or_else(|| get_most_out_of_order(home))
        .cloned()
}

fn get_earlier_build<'a>(home: &'a Shelter) -> Option<&'a Request> {
    home.requests()
        .filter(|r| matches!(&r.kind, RequestKind::Build(_) if
...
fn get_most_out_of_order<'a>(home: &'a Shelter) -> Option<&'a Request> {
    home.requests()
        .filter(|r| matches!(&r.kind, RequestKind::Repair(_) if
            matches!(*r.status(), Status::Created)))
        .min_by(|first, second| {
            match &first.kind {
                RequestKind::Repair(first) => {
                    match &second.kind {
                        RequestKind::Repair(second) => {
                            first.hits.cmp(&second.hits)
                        }
