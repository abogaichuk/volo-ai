...
use log::*;
use crate::{
    rooms::{is_extractor, wrappers::farm::Farm},
    commons::is_cpu_on_low,
    movement::{callback::{closest_multi_rooms_range, construction_multi_rooms}, find_many, find_path}
};
use super::{RoomPlan, RoomPlannerError, PlannedCell, RoomStructure, RoomPart};
use std::{cmp::Ordering, collections::HashMap, str::FromStr};
use screeps::{HasPosition, Position, RoomName, RoomXY, game, pathfinder::SearchGoal};
...
impl Farm {
    pub fn plan_room(
        ...
        let mut existed_roads: HashMap<Position, usize> = existed_plans.iter()
            .flat_map(|(name, plan)| plan.costed_roads(*name))
            .collect();

        let mut structures:HashMap<RoomName, Vec<RoomXY>> = existed_plans.iter()
            .map(|(name, plan)| (*name, plan.unwalkable_structures()))
            .collect();

        let base_internal = *existed_roads.iter()
            .find_map(|(pos, distance)| if *distance == 0 {Some(pos)} else {None})
            .ok_or(RoomPlannerError::RoadConnectionFailure)?;

        for (target, _) in targets.into_iter()
            .map(|target| {
        ...
        {
            let max_distance = existed_roads.values()
                .max().copied().unwrap_or_default();
            ...
            while min < max_distance {
                ...
                let goals = existed_roads.iter()
                    .filter_map(|(pos, cost)| {
                        if *cost <= max && *cost >= min { Some(SearchGoal::new(*pos, 0)) } else { None }
                    });

                let search_result = find_many(
                    target,
                    goals,
                    construction_multi_rooms(&structures));
                ...
                let mut path = search_result.path().into_iter()
                    .filter(|pos| !pos.is_room_edge());
                let distance = path.next_back()
                ...
                let (shortest, new_min, new_max) = if let Some(costed_route) = best_route.take() {
                    debug!("comparing distance: {}, len: {}, last: {:?}, with current distance: {}, len: {}, last: {:?}",
                        distance,
                        route.path.len(),
                    ...
                    match costed_route.cmp(&route) {
                        Ordering::Less => {
                            debug!("less case: min: {}, max: {}, costed_route.dist: {}", min, max, costed_route.distance);
                            (costed_route, min.saturating_add(STEP), max.saturating_add(STEP))
                        },
                        Ordering::Equal => {
                            debug!("equal case: min: {}, max: {}, costed_route.dist: {}", min, max, costed_route.distance);
                            (costed_route, min.saturating_add(STEP), max.saturating_add(STEP))
                        },
                        Ordering::Greater => {
                            debug!("greater case: min: {}, max: {}, route.dist: {}", min, max, route.distance);
                ...
                info!(
                    ...
                    self.get_name(), cpu_used, new_min, new_max, shortest.distance,
                    shortest.path.len(),
            ...
            if let Some(costed_route) = best_route.take() {
                info!("from: {} - distance = {} + {}", target, costed_route.distance, costed_route.path.len() - 1);
                let r = costed_route.path.iter()
                    .fold(String::from_str("").expect("expect str"), |acc, elem| {
                ...
                structures.entry(container.room_name())
        ...
        let mut plans: HashMap<RoomName, RoomPlan> = existed_roads.into_iter()
            .fold(
                HashMap::new(),
                |mut acc, (pos, cost)| {
                    acc.entry(pos.room_name())
                        .or_default()
                        .add_cell(PlannedCell::new(pos.xy(), RoomStructure::Road(cost), 0, None));
                    acc
                },
            );

        for xy in structures.get(&self.get_name())
            .ok_or(RoomPlannerError::ContainerPlacementError)?.iter()
        {
            let cell = PlannedCell::new(*xy, RoomStructure::Container(RoomPart::Red), 0, None);
            plans.entry(self.get_name())
                .and_modify(|plan| plan.add_cell(cell));
        }
