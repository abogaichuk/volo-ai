...
use crate::rooms::state::constructions::owned::{CentralSquare, Square};
use crate::rooms::state::constructions::xy_util::counter_clockwise_dir;
use crate::rooms::state::constructions::{
    PlannedCell, RoomPlan, RoomPlannerError, RoomStructure, Walls, is_wall,
};

pub fn plan(spawns: &[RoomXY], room_plan: &mut RoomPlan) {
    room_plan.add_cells(spawns.iter().enumerate().map(|(i, xy)| {
        let ctrl_lvl = if i == 0 {
            1
        } else if i == 1 {
            7
...
pub fn spawn_space(
    ...
    squares: &[Square],
    walls: &Walls,
) -> Result<Vec<RoomXY>, RoomPlannerError> {
    ...
    let direction = match central.guide_dir() {
        ...
        | Direction::TopLeft
        | Direction::TopRight => -central.guide_dir(),
    ...
    let candidates: Vec<RoomXY> = squares
        .iter()
