...
use std::{iter, cmp::Reverse, collections::HashMap};
use crate::rooms::state::constructions::{PlannedCell, RoomPart, RoomPlan, RoomStructure, owned::Perimeter};
    ...
    room_plan: &mut RoomPlan)
{
    ...
    let candidates: Vec<(RoomXY, usize)> = grid.iter()
        .filter_map(|(xy, part)| if part.is_yellow() { Some(*xy) } else { None })
        .map(|yellow| {
            let under_attack_count = reds.iter()
                .filter(|red| red.get_range_to(yellow) == 3)
                .count();
    ...
    room_plan.add_cells(
        select_best_spread(candidates, 6, 4)
            .enumerate()
            .flat_map(|(i, xy)| {
                let ctrl_lvl = if i == 0 {
                    3
...
/// Exact selector: maximize total score with pairwise Chebyshev distance > min_exclusive.
/// Returns up to `k` items (fewer if constraints force it).
