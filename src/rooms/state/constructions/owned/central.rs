...
use crate::rooms::state::constructions::xy_util::{
    clockwise_dir, counter_clockwise_dir, square_sides,
};
use crate::rooms::state::constructions::{
    LabStatus, LinkType, PlannedCell, RoomPlannerError, RoomStructure, Walls,
};
...
pub fn central_square(
    ...
    walls: &Walls,
) -> Result<CentralSquare, RoomPlannerError> {
    let mut places = Vec::new();

    let centres: HashSet<RoomXY> =
        squares.iter().filter(|square| square.rounded()).map(|square| *square.center()).collect();
    ...
    for road in roads {
        ...
        if neighbor_centres.iter().all(|xy| centres.contains(&xy.1)) {
            let squares: HashMap<Direction, Square> = neighbor_centres
                .into_iter()
                .map(|(dir, center)| {
...
impl CentralSquare {
    ...
    pub fn plan(&self) -> Result<HashSet<PlannedCell>, RoomPlannerError> {
        ...
        let storage_square = self
            .squares
            .get(&storage_sqr_dir)
            .ok_or(RoomPlannerError::CentralSquarePlacementError)?;
        let storage = PlannedCell::new(
            storage_square.center.saturating_add_direction(terminal_sqr_dir), /* storage close
                                                                               * to terminal */
        ...
        let road = PlannedCell::new(storage_square.center, RoomStructure::Road(0), 1, None);
        ...
        let terminal_square = self
            .squares
            .get(&terminal_sqr_dir)
            .ok_or(RoomPlannerError::CentralSquarePlacementError)?;
        let terminal = PlannedCell::new(
            terminal_square.center.saturating_add_direction(storage_sqr_dir), /* terminal close
                                                                               * to storage */
        ...
        let road = PlannedCell::new(terminal_square.center, RoomStructure::Road(0), 1, None);
        ...
        let nuker_square = self
            .squares
            .get(&nuker_sqr_dir)
            .ok_or(RoomPlannerError::CentralSquarePlacementError)?;
        let nuker = PlannedCell::new(
            nuker_square.center.saturating_add_direction(nuker_sqr_dir), /* opposite direction
                                                                          * from storage */
            RoomStructure::Nuker,
            8,
            None,
        ...
        let lab_ouput3 =
            PlannedCell::new(nuker_square.center, RoomStructure::Lab(LabStatus::Output), 7, None);
        ...
        let ps_square =
            self.squares.get(&ps_sqr_dir).ok_or(RoomPlannerError::CentralSquarePlacementError)?;
        let ps = PlannedCell::new(
            ps_square.center.saturating_add_direction(ps_sqr_dir), /* opposite direction from
                                                                    * terminal */
        ...
        let lab_ouput6 =
            PlannedCell::new(ps_square.center, RoomStructure::Lab(LabStatus::Output), 8, None);
