use screeps::{pathfinder::SearchGoal, Position, RoomXY};
use std::collections::HashMap;
use crate::{
    movement::{callback::construction_single_room, find_many_in_room},
    rooms::state::constructions::{
        LinkType, PlannedCell, RoomPart, RoomPlan, RoomPlannerError, RoomStructure
    }
};
use super::{walkable_neighbors, place_for_container};

pub fn plan(
    storage: Position,
    ctrl: RoomXY,
    grid: &HashMap<RoomXY, RoomPart>,
    room_plan: &mut RoomPlan) -> Result<(), RoomPlannerError>
{
    ...
    let container = place_for_container(
            ...
            .and_then(|xy| grid.get(&xy)
                .map(|part| PlannedCell::new(
                    xy,
    ...
    let road = search_result.path().into_iter().rev().enumerate()
        .map(|(i, step)| {
...
mod tests {
    ...
    //     let cells = plan(&cross_road, &ctrl, &grid).expect("expect ctrl planned cells");

    //     assert_eq!(
    //         cells.iter().filter(|c| matches!(c.structure, RoomStructure::Rampart(_))).count(),
    //         4,
    //         "expect 4 ramparts"
    //     );
    //     assert_eq!(
    //         cells.iter().filter(|c| matches!(c.structure, RoomStructure::Link(_))).count(),
    //         1,
    //         "expect 1 link"
    //     );
    //     assert_eq!(
    //         cells.iter().filter(|c| matches!(c.structure, RoomStructure::Container(_))).count(),
    //         1,
    //         "expect 1 container"
    //     );
    // }
