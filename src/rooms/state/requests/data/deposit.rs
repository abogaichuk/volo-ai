...
use crate::{
    units::roles::{Role, teams::{dep_miner::DepositMiner, dep_hauler::DepositHauler}},
    rooms::{RoomEvent, state::requests::{Meta, Status, Assignment}},
    utils::commons::has_part
};
    ...
    match meta.status {
        Status::Created => {
            if let Some(squad_id) = assignment.new_squad(data.id.to_string(), meta) {
                ...
                let dep_miner = Role::DepositMiner(DepositMiner::new(Some(squad_id.clone()), Some(home_name)));
                let dep_hauler = Role::DepositHauler(DepositHauler::new(Some(squad_id.clone()), Some(home_name)));
        ...
        Status::InProgress => {
            if let Some(deposit) = data.id.resolve() && deposit.last_cooldown() >= DEPOSIT_REQUEST_THRESHOLD {
                meta.update(Status::Carry);
            } else if game::time() > meta.updated_at + 1350 {
                let fast_spawn = game::rooms().get(data.pos.room_name())
                    .map(|room| room.find(find::HOSTILE_CREEPS, None).iter()
                        .any(|hostile| has_part(&[Part::Work], hostile, false) &&
