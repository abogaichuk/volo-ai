...
use crate::rooms::{state::constructions::{LinkType, PlannedCell, RoomPlan, RoomStructure}, wrappers::claimed::Claimed};

impl Claimed {
    pub(crate) fn run_links(&self) {
        if let Some(farm_link) = self.links.other.iter()
        ...
        if let Some(ctrl_link) = self.links.ctrl() {
            if ctrl_link.store().get_free_capacity(Some(ResourceType::Energy)) > 500 &&
                let Some(sender) = self.links.sender()
        ...
        for link in links {
            ...
            if let Some(planned_cell) = plan.get_cell(cell) {
                match planned_cell.structure {
                    ...
                    _ => { warn!("invalid plan search for: {}", link.pos()); }
                }
            }
        }
