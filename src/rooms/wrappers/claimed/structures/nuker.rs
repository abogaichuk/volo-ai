...
impl Claimed {
    pub(crate) fn run_nuker(&self) -> Option<Request> {
        self.nuker.as_ref()
            .and_then(|nuker| {
                if nuker.store().get_used_capacity(Some(ResourceType::Energy)) < nuker.store().get_capacity(Some(ResourceType::Energy)) {
                    find_container_with(ResourceType::Energy, Some(150000), self.storage(), None, None)
                        .map(|(id, _)| Request::new(
                            RequestKind::Carry(CarryData::new(
                                ...
                                nuker.raw_id(),
                                ResourceType::Energy,
                                min(nuker.store().get_free_capacity(Some(ResourceType::Energy)) as u32, MAX_CARRY_REQUEST_AMOUNT))),
                    ...
                    if nuker_amount < GHODIUM_LOAD_CAPACITY {
                        if let Some(storage) = self.storage() {
                            let storage_amount = storage.store().get_used_capacity(Some(ResourceType::Ghodium));
                            if storage_amount > 0 {
