use log::*;
use screeps::{ResourceType, SpawnOptions, StructureProperties, StructureSpawn, action_error_codes::SpawnCreepErrorCode, game};
use smallvec::SmallVec;
use crate::{
    rooms::{state::RoomState, RoomEvent, wrappers::claimed::Claimed},
...
impl Claimed {
    ...
    pub(crate) fn run_spawns(&self, room_memory: &RoomState) -> SmallVec<[RoomEvent; 2]> {
        ...
        if !room_memory.spawns.is_empty() {
            if room_memory.spawns.len() > 1 && !room_memory.powers.contains(&screeps::PowerType::OperateSpawn) {
    ...
    fn try_spawn(&self, room_memory: &RoomState) -> Option<RoomEvent> {
        get_spawn_data_by_priority(&room_memory.spawns)
            .and_then(|(index, spawn_role)| {
                if let Some(spawn) = self.find_available_spawn() {
                    let max_scale = self.storage()
                        .map(|storage| storage.store()
                            .get_used_capacity(Some(ResourceType::Energy)) > MIN_ENERGY_AMOUNT && !room_memory.origin)
                        .unwrap_or(!room_memory.origin);

                    let room_energy = if max_scale { self.energy_capacity_available() } else { self.energy_available() };

                    if !max_scale {
                        warn!("{} spawn creeps with limited scales! room_energy {}", self.get_name(), room_energy);
                    }
                        ...
                        Ok(_) => {
                            ...
                            'spawn_loop: loop {
                                ...
                                match spawn.spawn_creep_with_options(&body, &name, &spawn_options) {
                                    Ok(_) => {
                                        debug!("room: {} successfully spawned creep: {}", self.get_name(), name);
                                        break 'spawn_loop Some(RoomEvent::Spawned(name, spawn_role.clone(), index));
                                    },
                                    Err(err) => match err {
                                        SpawnCreepErrorCode::NameExists => { sufix += 1; },
                                        _ => { break 'spawn_loop None; }
                                    }
                                }
                            }
                        }
                        Err(err) => {
                            let message = format!("{} can't spawn role: {:?}, error: {:?}", self.get_name(), spawn_role, err);
                            debug!("{}", message);
                            None
                        }
                    }
    ...
    fn find_available_spawn(&self) -> Option<&StructureSpawn> {
        self.spawns.iter()
            .find(|s| s.spawning().is_none() && s.is_active())
    }
}
...
fn get_spawn_data_by_priority(spawns: &[Role]) -> Option<(usize, &Role)> {
    spawns.iter()
        .enumerate()
        .fold(None, |result:Option<(usize, &Role)>, (index, spawn_role)| -> Option<(usize, &Role)> {
            result
                .map(|result| {
