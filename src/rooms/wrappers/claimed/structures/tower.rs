...
impl Claimed {
    pub(crate) fn run_towers(&self) {
        ...
        let (hostiles, invanders):(Vec<_>, _) = self.hostiles.iter()
            .partition(|enemy| enemy.owner().username() != INVADER_USERNAME);

        if !hostiles.is_empty() {
            let (boosted, unboosted):(Vec<&&Creep>, _) = hostiles.iter()
                .partition(|hostile| full_boosted(hostile));

            if !boosted.is_empty() {
                let healers_dont_have = [Part::Attack, Part::RangedAttack, Part::Work];
                let (healers, another):(Vec<&&Creep>, _) = boosted.iter()
                    .partition(|hostile| hostile.body().iter()
                        .all(|body_part| !healers_dont_have.contains(&body_part.part())));

                let (_can_heal, without_heal):(Vec<&&Creep>, _) = another.iter()
                    .partition(|hostile| hostile.body().iter()
                        .any(|body_part| body_part.part() == Part::Heal));

                
                if let Some(target) = without_heal.iter()
                    .find(|hostile| self.towers.iter().any(|tower| tower.pos().get_range_to(hostile.pos()) <= 5) &&
                        healers.iter().all(|healer| healer.pos().get_range_to(hostile.pos()) > 2))
                {
                    self.mass_attack(target);
                } else if let Some(injured) = boosted.iter().find(|hostile| hostile.hits() < hostile.hits_max()) {
                    self.mass_attack(injured);
                } else if let Some(closest) = boosted.iter().find(|enemy| is_near_to(enemy, &perimetr)) &&
    ...
    fn distributed_heal<'a, I>(&self, injured_creeps: I)
        where I: Iterator<Item = &'a dyn Healable> 
    {
...
fn find_all_injured<'a>(creeps: &'a [Creep], p_creeps: &'a [PowerCreep]) -> impl Iterator<Item = &'a dyn Healable> {
    creeps.iter()
        .filter(|creep| creep.hits() < creep.hits_max())
        .map(|creep| creep as &dyn Healable)
        .chain(p_creeps.iter()
            .filter(|pc| pc.hits() < pc.hits_max())
            .map(|pc| pc as &dyn Healable))
}