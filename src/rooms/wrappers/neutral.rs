use log::*;
use screeps::{find, game, Creep, Deposit, EffectType, HasHits, HasId, HasPosition,
    OwnedStructureProperties, ResourceType, Room, RoomName, RoomObjectProperties,
    SharedCreepProperties, StructureController, StructureInvaderCore,StructureObject,
    StructurePowerBank, Tombstone, SYSTEM_USERNAME
};
use std::{cmp, collections::BTreeMap};
use crate::{
    colony::ColonyEvent,
    commons::{get_room_regex, capture_room_numbers, is_skr, is_highway, find_walkable_positions_near_by, is_near_edge},
    utils::constants::DEPOSIT_REQUEST_THRESHOLD
};
...
impl Neutral {
    pub fn new(room: Room) -> Self {
        ...
        let icore = room.find(find::HOSTILE_STRUCTURES, None)
            .into_iter()
            .find_map(|structure| {
                match structure {
        ...
        let power_banks = room.find(find::STRUCTURES, None)
            .into_iter()
        ...
        let is_blocked = room.find(find::STRUCTURES, None)
            .into_iter()
            .any(|structure| matches!(structure, StructureObject::StructureWall(w) if w.hits() == 0));
            ...
            if let Some((f_num, s_num)) = capture_room_numbers(&re, self.room_name) {
                ...
                if is_skr(f_rem, s_rem) {
                    ...
                    let ic_timeout = self.icore.as_ref()
                        ...
                        .unwrap_or_default();
                    if ic_timeout > 0 {
                        events.push(ColonyEvent::AvoidRoom(self.room_name, game::time() + ic_timeout));
                    }
                ...
                } else if let Some(controller) = self.controller.as_ref() && controller.level() > 0 {
                    //not for farm rooms with controller, ally, neutral, hostiles or reserved by somebody else
    ...
    fn withdraw_events(&self) -> impl Iterator<Item = ColonyEvent> + '_ {
        self.tombs.iter()
            .filter_map(|tomb| {
                ...
                if amount > 500 {
                    Some(ColonyEvent::Withdraw(tomb.raw_id(), tomb.pos(), ResourceType::CatalyzedGhodiumAcid, amount))
                } else {
                    None
                }
            })
    ...
    fn caravan_event(&self) -> Option<ColonyEvent> {
        let screeps: Vec<&Creep> = self.enemies.iter()
            .filter(|hostile| hostile.owner().username() == SYSTEM_USERNAME)
            .collect();
    
        if !screeps.is_empty() &&
            screeps.iter().all(|hostile| !hostile.pos().is_room_edge() &&
            !is_near_edge(hostile.pos()))
        {
            let creeps: BTreeMap<String, u32> = screeps.into_iter()
